...
----> 데이터타입 수정할 경우 컬럼에 저장된 크기 미만으로는 변경할 수 없음
----> 데이터타입 VARCHAR2(100) -> VARCHAR2(10) 데이터타입이 10 초과되는건 불가
...
----> 컬럼값을 조절할 때 (큰값->작은값) 이미 저장된 데이터값이 변경할값보다 크지 않은지 확인필요
...
--> DEPT_TITLE 컬럼 1번방법으로 삭제
ALTER TABLE DEPT_COPY DROP (DEPT_TITLE);
SELECT * FROM DEPT_COPY;

--> LOCATION_ID 컬럼 2번방법으로 삭제
...
CREATE TABLE TB2(
    TB1_PK NUMBER PRIMARY KEY,
    TB2_COL NUMBER REFERENCES TB1 -- TB1테이블의 PK값을 참조
);
...
-- 외래키에 의해 참조되는 고유/기본키가 존재하기 때문에 삭제할 수 없음
-- 삭제하고 싶은데 어떻게 삭제를 하는가?
-- 해결방법1 : 자식테이블(TB2) 삭제후 부모테이블(TB1)을 삭제
DROP TABLE TB2;
DROP TABLE TB1;


-- 해결방법 2 : CASCADE CONSTRAINT 옵션 사용
--> 제약조건까지 모두 삭제, 조건을 없애버려서 부모자식을 끊음
