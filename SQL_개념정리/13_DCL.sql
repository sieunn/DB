/*
계정(사용자)
* 관리자계정 : 데이터베이스의 생성과 관리를 담당하느 계정
            모든 권한과 책임을 가지는 계정
            sys(최고관리자), system(sys에서 권한 몇 개 제외된 관리자)
            
* 사용자계정 : 데이터베이스에 대해 질의,갱신,보고서 작성 등의
            작업을 수행할 수 있는 계정으로
...
1) 시스템 권한 :DB 접속, 객체 생성 권한
    CREATE USER : 사용자(계정) 생성하는 권한
    CREATE TABLE : 테이블 생성 권한
    CREATE SESSTION : 데이터베이스 접속 권한
    CREATE VIEW : 뷰 생성권한
    CREATE SEQUENCE : 시퀀스 생성 권한
    CREATE PROCEDURE : 함수(프로시저) 생성권한
    DROP USER : 사용자계정 삭제 권한
    DROP ANY TABLE : 임의 테이블 삭제권한
...
2) GRANT / REVOKE 사용권한
    ...
    계정에 권한을 부여할 때 두가지 이상 부여할 경우 (,사용해서작성)
    GRANT 권한,권한 TO 계정명;
