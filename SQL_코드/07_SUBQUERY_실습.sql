...
SELECT AVG(SALARY)
FROM EMPLOYEE E
WHERE DEPT_CODE = E.DEPT_CODE; -- = E.DEPT_CODE; 의미가 없는 코드 없어도 됨

--2) 직원들의 이름과 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SARALY > 3089820;
...
-- 3. D5 부서의 평균급여보다 높은 급여를 받는 직원들의 이름 조회
-- SELECT EMP_NAME  AVG
SELECT EMP_NAME 
FROM EMPLOYEE
WHERE SALARY > (SELECT AVG(SALARY)
...
-- 4. 급여가 300만원 이상인 직원중에서 가장 낮은 급여를 받는 직원의 이름과 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
...
--> 서브쿼리를 통해서 특정 조건을 만족하는 최저급여를 찾고,
----> 조건에 맞는 급여를 받는 직원을 외부 커리에서 조회
...
-- 2. 부서별로 직원수가 5명이상인 부서의 부서명과 직원 수 조회
-- SELECT 서브쿼리
-- WHERE 서브쿼리
-- 외부쿼리 
SELECT DEPT_TITLE,
